    ...
    async componentDidMount() {
        ...
        const items = candidates.map(candidate => {
            ...
            return {
              ...
              image: (
  <Image id={i} src={'https://react.semantic-ui.com/images/avatar/large/matthew.png'} style={{maxWidth: '100%',maxHeight:'190px'}}/>
),
              extra: (
                  <div>
    ...
    onSubmit = async (event) => {
        ...
        // --- FIXED CODE START ---
        try {
            // 1. Skip the broken IPFS upload
            const fakeIpfsHash = "QcFakeHashForDemo123"; 
            this.setState({ ipfsHash: fakeIpfsHash });
                    
            const add = Cookies.get('address');
            const election = Election(add);

            // 2. Save directly to Blockchain
            ...
            // 3. If Blockchain succeeds, tell the server (and show alert)
